...
void nms(std::vector<Detection>& res, float *output, float conf_thresh, float nms_thresh,int num_bboxes) {
...
//     std::cout << "detected before nms -> " << output[0] << std::endl;
    ...
    for(int i=0; i< num_bboxes; i++) {
        if(output[conf_val] >= conf_thresh) {
            int idx = std::distance(output+conf_val+1, std::max_element(output+conf_val+1, output+ conf_val + 80+1));
...
//     std::cout << output.inferDims.d[0] << std::endl;
//     std::cout << output.inferDims.d[1] << std::endl;
//     std::cout << output.inferDims.numDims << std::endl;
//     std::cout << boxes.inferDims.numElements << std::endl;
    
    nms(res, (float*)(outputLayersInfo[0].buffer), kCONF_THRESH, kNMS_THRESH, num_bboxes);
